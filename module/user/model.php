...
class userModel extends model
{
    ...
    public function batchCreate()
    {
        ...
        for($i = 0; $i < $this->config->user->batchCreate; $i++)
        {
            if($users->account[$i] != '')  
            {
                ...
                $data[$i]->leader   = $users->leader[$i] == 'ditto' ? (isset($prev['leader']) ? $prev['leader'] : '') : $users->leader[$i];
                $data[$i]->role     = $role;
    ...
    public function batchEdit()
    {
        ...
        foreach($this->post->account as $id => $account)
        {
            ...
            $users[$id]['leader']     = $this->post->leader[$id] == 'ditto' ? (isset($prev['leader']) ? $prev['leader'] : 0) : $this->post->leader[$id];
            ...
            if(empty($users[$id]['leader'])) die(js::error(sprintf($this->lang->user->error->leader, $id)));
            
            $accounts[$id] = $account;
            $prev['dept']  = $users[$id]['dept'];
            $prev['role']  = $users[$id]['role'];
            $prev['leader']  = $users[$id]['leader'];
        }
